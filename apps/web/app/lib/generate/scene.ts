...
function getOpenAIClient(apiKey: string | undefined) {
  if (!apiKey) {
...
export async function generateScene(request: SceneRequest, keys: GenerateKeys) {
  ...
  if (
    ...
    return generateSceneViaInpaint(request, keys);
  }

  if (request.provider === "gemini") {
    return generateSceneWithGemini(request, keys);
  }

  return generateSceneWithOpenAI(request, keys);
}
...
async function generateSceneViaInpaint(
  ...
  const openai = getOpenAIClient(keys.openaiApiKey);
...
async function generateSceneWithOpenAI(request: SceneRequest, keys: GenerateKeys) {
  const openai = getOpenAIClient(keys.openaiApiKey);
...
async function generateSceneWithGemini(
  ...
  const ai = getGeminiClient(keys.geminiApiKey);
...
export function generatedImageToBytes(result: GeneratedImageResult): ArrayBuffer {
